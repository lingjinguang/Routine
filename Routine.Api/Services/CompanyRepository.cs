...
namespace Routine.Api.Services
{
    public class CompanyRepository:ICompanyRepository
    {
        ...
        public async Task<IEnumerable<Company>> GetCompaniesAsync(IEnumerable<Guid> compnyIds)
        {
            if(compnyIds == null)
            {
                throw new ArgumentNullException(nameof(compnyIds));
            }
            return await _context.Companies.Where(x=>compnyIds.Contains(x.Id)).ToListAsync();
        }
        ...
        public async Task<IEnumerable<Employee>> GetEmployeesAsync(Guid companyId, EmployeeDtoParameters parameters)
        {
            ...
            //{
            //    return await items.OrderBy(x => x.EmployeeNo).ToArrayAsync();
            //}
            ...
            if (!string.IsNullOrWhiteSpace(parameters.Q))
            {
                ...
                items = items.Where(x => x.FirstName.Contains(parameters.Q) || x.LastName.Contains(parameters.Q) || x.EmployeeNo.Contains(parameters.Q));
            }
            //if (!string.IsNullOrWhiteSpace(parameters.OrderBy))
            //{
            //    if (parameters.OrderBy.ToLowerInvariant() == "name")
            //    {
            //        items.OrderBy(x => x.FirstName).ThenBy(x=>x.LastName);
            //    }
            //}
